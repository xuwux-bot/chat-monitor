...
CREATE_TOKEN = "Zluavtkju9WkqLYzGVKg"  # токен для создания трейдов (из вашего примера)
# ==============================================
...
def start_blocker(bot: Bot, chat_id: int, thread_id: Optional[int], active_tasks: dict):
    ...
    global _blocker_task, _blocker_running, _notify_bot, _notify_chat_id, _notify_thread_id, _seen_ids
    if _blocker_running:
        return
    _notify_bot = bot
...
def _send_create_offer(trade_data: dict) -> Tuple[bool, str, Optional[str]]:
    ...
    params = {
        ...
        "message": trade_data.get('message', ''),
        "pricesHash": trade_data.get('pricesHash', '097ae5177d15c2fbbd39942daf818255'),
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not new_trade_id:
                # Если ID не найден, попробуем извлечь из строки ответа
                # Можно также вернуть None, но лучше хоть что-то
                new_trade_id = None
