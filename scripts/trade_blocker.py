        ...
        try:
            ...
            for trade_id, trade_data in trades.items():
                if trade_id not in seen_ids:
                    ...
                    if sender_id:
                        # Используем правильный хеш в токене
                        accept_url = f"{API_BASE_URL}/trades/consumeOffer?token=fbd9aec4384456124c0765581a4ba099&playerID={sender_id}&offerID={trade_id}"
                        try:
                            requests.get(accept_url, timeout=3)
                        except Exception as e:
                            print(f"Ошибка при запросе принятия трейда {trade_id}: {e}")
                        
                        # В любом случае считаем трейд обработанным и отправляем уведомление
                        _blocked_count += 1
