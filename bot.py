...
async def item_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "withdraw":
        ...
        if not game_id:
            ...
            logger.error(f"У игрока {owner_id} нет game_id")
            return

        # Генерируем случайный токен для этого запроса
        random_token = generate_random_token()
        logger.info(f"Сгенерирован случайный токен: {random_token}")

        # Формируем URL в точности как в предоставленном скрипте, но со случайным токеном
        base_url = "https://api.efezgames.com/v1/trades/createOffer"
        full_url = (f"{base_url}?token={random_token}"  # <-- случайный токен
                    f"&playerID={game_id}"
                    f"&receiverID="
...
async def handle_activate_promo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rewards:
        # Для обратной совместимости, если промокод старый с одним reward
        old_reward = promo.get('reward')
        if old_reward:
            rewards = [old_reward]
...
async def promo_create_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Возвращаем пользователя в меню игрока
    await show_user_profile(update, context)
...
async def handle_reply_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка кнопок создания промокода
