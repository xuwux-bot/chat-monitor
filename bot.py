...
def pemoji(ch: str) -> str:
    eid = PREMIUM_EMOJI.get(ch)
    if not eid:
        return ch
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text(
        f"{pemoji('👤')} Выберите игровой чат для подтверждения:",
        reply_markup=reply_markup,
        parse_mode="HTML"
    )
...
async def confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'reg_chat' not in context.user_data or 'reg_code' not in context.user_data:
        await update.message.reply_text(f"{pemoji('⚠️')} Сначала выполните /start и выберите чат")
        return
    ...
    if result and result.get("success"):
        ...
        for tid, pdata in players.items():
            if pdata.get('game_id') == game_id:
                await update.message.reply_text(f"{pemoji('❌')} Этот игровой ID уже зарегистрирован другим пользователем.")
                return
