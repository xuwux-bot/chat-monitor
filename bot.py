...
# ============= ДРУЗЬЯ =============
async def friends_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает inline-меню для управления друзьями."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    players = load_json(PLAYERS_FILE, {})
    ...
    keyboard = [
        [InlineKeyboardButton("🧟‍♂️ Добавить друга", callback_data="friend_add")],
        [InlineKeyboardButton("👤 Список друзей", callback_data="friend_list")],
        [InlineKeyboardButton("🕓 Активные запросы", callback_data="friend_requests")],
        [InlineKeyboardButton("◀️ Назад в профиль", callback_data="back_to_profile")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text("👤 **Управление друзьями**\nВыберите действие:", reply_markup=reply_markup, parse_mode='Markdown')

async def friend_add_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Запускает процесс добавления друга."""
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    awaiting_friend_add[user_id] = True
    await query.edit_message_text("• Вы хотите добавить друга, введите ник вашего друга.\n(Друг должен быть зарегистрирован в боте)\n\n| Введите ник:")
...
async def friend_list_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает список друзей с inline-кнопками для каждого."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    players = load_json(PLAYERS_FILE, {})
    ...
    if not friends:
        keyboard = [[InlineKeyboardButton("◀️ Назад", callback_data="friends_menu")]]
        await query.edit_message_text("У вас пока нет друзей", reply_markup=InlineKeyboardMarkup(keyboard))
        return
    ...
    for friend_nick in friends:
        ...
        keyboard.append(row)
    keyboard.append([InlineKeyboardButton("◀️ Назад", callback_data="friends_menu")])
...
async def friend_profile_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("◀️ Назад к списку друзей", callback_data="friend_list")]]
    await query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
...
async def friend_delete_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад к списку друзей", callback_data="friend_list")]])
    )

async def friend_requests_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает список входящих запросов в друзья."""
    query = update.callback_query
    await query.answer()
    user_id = str(query.from_user.id)
    players = load_json(PLAYERS_FILE, {})
    ...
    if not requests:
        keyboard = [[InlineKeyboardButton("◀️ Назад", callback_data="friends_menu")]]
        await query.edit_message_text("У вас нет активных запросов в друзья", reply_markup=InlineKeyboardMarkup(keyboard))
        return
...
async def back_to_profile_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    query = update.callback_query
    await query.answer()
    # Просто показываем профиль заново (но это callback, поэтому нужно создать Update для имитации)
    # Можно отредактировать сообщение или отправить новое. Лучше отредактировать, но для простоты отправим новое сообщение.
    await query.message.delete()
    # Теперь нужно вызвать show_user_profile, но у нас нет update.message. Можно создать искусственный Update?
    # Проще: отправить новое сообщение, но для этого нужно иметь chat_id.
    # Используем context.bot.send_message и затем вызовем show_user_profile, передав ей update.message?
    # Пока сделаем так: отправим новое сообщение и покажем профиль через отдельную функцию.
    user_id = query.from_user.id
    players = load_json(PLAYERS_FILE, {})
    pdata = players.get(str(user_id), {})
    coins = pdata.get('coins', 0)
...
# ============= ПРОФИЛЬ ПОЛЬЗОВАТЕЛЯ (УКОРОЧЕННЫЙ) =============
@require_registration
@check_ban
async def show_user_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Клавиатура: только кнопка "Друзья" и "Реферальная система"
    keyboard = [
        [KeyboardButton("👤 Друзья")],
        [KeyboardButton("⚡ Реферальная система")]
    ]

    # Для админов добавляем кнопку "◀️ Назад" (возврат к выбору меню)
    if is_admin_or_owner(int(user_id)):
        keyboard.append([KeyboardButton("◀️ Назад")])
...
async def handle_reply_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "👤 Меню игрока":
        ...
        # Для админов: возврат к выбору меню
        if is_admin_or_owner(user_id):
            keyboard = [
                [KeyboardButton("👤 Меню игрока")],
                [KeyboardButton("⚙️ Админ-меню")]
            ...
            # Для обычных (у которых есть кнопка Назад? Нет, но на всякий случай)
            await show_user_profile(update, context)
    else:
        pass
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["👤 Меню игрока", "⚙️ Админ-меню", "👤 Друзья", "⚡ Реферальная система", "👤 Мой профиль (админ)", "🔍 Найти игрока", "◀️ Назад"]:
        if not is_registered(user_id):
            await update.message.reply_text("❌ Сначала зарегистрируйтесь через /start")
            return
    ...
    # Обработка сообщений в ветках мониторинга (отправка в игру)
    ...
    # Обработка выбора языка для PREMIUM
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(friend_list_callback, pattern='^friend_list$'))
    app.add_handler(CallbackQueryHandler(friend_requests_callback, pattern='^friend_requests$'))
    app.add_handler(CallbackQueryHandler(friend_add_callback, pattern='^friend_add$'))
    app.add_handler(CallbackQueryHandler(friends_menu_callback, pattern='^friends_menu$'))
    app.add_handler(CallbackQueryHandler(back_to_profile_callback, pattern='^back_to_profile$'))
