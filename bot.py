...
async def handle_view_other_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Формируем профиль (аналогично show_user_profile, но без клавиатуры)
    nick_emoji = '<tg-emoji emoji-id="5210956306952758910">🗨️</tg-emoji>'
    ...
    text = (
        f"👤 <b>Профиль игрока {html.escape(target_nick)}</b>\n"
        f"{nick_emoji} Никнейм в игре: {html.escape(target_data.get('game_nick', 'неизвестно'))}\n"
        f"{role_emoji} Роль: {'Игрок' if target_data.get('role')=='user' else 'Администратор'}\n"
        f"{time_emoji} Дата регистрации: {html.escape(target_data.get('registered_at', 'неизвестно'))}\n"
...
async def update_promo_message(bot, code: str, promo: dict):
    """Обновляет сообщение о промокоде в канале, если есть message_id."""
    ...
    try:
        ...
        print(f"Ошибка обновления сообщения промокода {code}: {e}")
...
async def everyone_send(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    for tid in players.keys():
        try:
            ...
            print(f"Не удалось отправить сообщение пользователю {tid}: {e}")
            failed_count += 1
    broadcasts = load_broadcasts()
    broadcasts[code] = {
        "code": code,
...
async def everyone_delete(update: Update, context: ContextTypes.DEFAULT_TYPE, code: str):
    ...
    for tid, msg_id in data['messages'].items():
        try:
            ...
            print(f"Не удалось удалить сообщение у {tid}: {e}")
            errors += 1
    del broadcasts[code]
    save_broadcasts(broadcasts)
    await update.message.reply_text(
