...
async def item_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ============= ВЫВОД СКИНА (ИСПРАВЛЕННЫЙ БЛОК) =============
    if action == "withdraw":
        # Генерируем уникальное сообщение для трейда (код)
        msg_code = '#' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))

        # Формируем строку скина (skin) – это то, что будет подставлено в параметр skinsOffered
        skin = format_skin_for_trade(item)   # ← вот она, переменная skin, как в примере

        # Получаем game_id игрока (получателя)
        players = load_json(PLAYERS_FILE, {})
        ...
        # Формируем URL в точности как в предоставленном скрипте
        base_url = "https://api.efezgames.com/v1/trades/createOffer"
        full_url = (f"{base_url}?token={GAME_API_TOKEN}"
                    f"&playerID={game_id}"
                    f"&receiverID="                     # оставляем пустым
                    f"&senderNick=EfezBot"
                    f"&senderFrame="                    # можно добавить значения, если нужно
                    f"&senderAvatar="                   #
                    f"&receiverNick="                    #
                    f"&receiverFrame="                   #
                    f"&receiverAvatar="                  #
                    f"&skinsOffered={urllib.parse.quote(skin)}"   # экранируем спецсимволы
                    f"&skinsRequested="                  # пусто, мы только отправляем
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if trade_id:
                    ...
                    add_to_whitelist(
                        ...
                        msg_code,
                        game_id,
                        skin,   # здесь передаём skin
                        notification_msg_id=sent.message_id if sent else None
                    )
            ...
            logger.error(f"Исключение при выводе: {e}", exc_info=True)
...
async def promo_create_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(format_promo_info(promo_entry))
...
# ============= НОВАЯ КОМАНДА /id (ПОИСК ИГРОКА ПО ID В ИГРЕ) =============
async def id_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Получаем основную информацию через findUser
    try:
    ...
    # Получаем экипировку (инвентарь) через getEQ
    eq_data = {}
    try:
    ...
    # Заблокированные пользователи (если есть)
...
# ============= ОБРАБОТЧИК ДЛЯ КНОПКИ ИНВЕНТАРЯ ИЗ /id =============
async def admin_inventory_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(data) < 2:
        ...
        return
    target_id = data[1]
    # Показываем инвентарь в режиме admin
    context.user_data['last_inventory_target'] = target_id
...
# ============= HELP (с добавленными командами /everyone и /id) =============
...
def main():
    ...
    app.add_handler(CommandHandler("id", id_command))  # Новая команда
    app.add_handler(CallbackQueryHandler(admin_inventory_callback, pattern='^admin_inventory\|'))  # Обработчик для кнопки
