...
async def back_handler(update, context):
    ...
    try:
        if is_admin_or_owner(query.from_user.id):
            await context.bot.send_message(chat_id=query.from_user.id, text="Выберите меню:", reply_markup=ReplyKeyboardMarkup([[KeyboardButton("👤 Меню игрока")],[KeyboardButton("⚙️ Админ-меню")]], resize_keyboard=True))
        else:
            await show_user_profile(update, context)
    except Exception:
        await context.bot.send_message(chat_id=query.from_user.id, text="Ошибка возврата в меню")
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(chat_selected, pattern=r'^reg_chat_'))
    ...
    app.add_handler(CallbackQueryHandler(toggle_auto_friend_callback, pattern=r'^toggle_auto_friend$'))
    app.add_handler(CallbackQueryHandler(inventory_navigation_callback, pattern=r'^(nav|item|back_to_)'))
    ...
    app.add_handler(CallbackQueryHandler(toggle_friend_requests_callback, pattern=r'^toggle_friend_requests$'))
    app.add_handler(CallbackQueryHandler(toggle_accept_trades_callback, pattern=r'^toggle_accept_trades$'))
    app.add_handler(CallbackQueryHandler(profile_cost_callback, pattern=r'^profile_cost_'))
    app.add_handler(CallbackQueryHandler(promo_time_callback, pattern=r'^promo_time_'))
    app.add_handler(CallbackQueryHandler(promo_uses_callback, pattern=r'^promo_uses_'))
