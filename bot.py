...
# Чат для уведомлений о трейдах (блокировщик)
...
# Импортируем модули (убедитесь, что папка scripts существует и содержит __init__.py)
...
async def handle_find_player(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"• Ник в игре: {html.escape(player_data.get('game_nick', 'неизвестно'))}\n"
        f"• Айди игрока в игре: {html.escape(player_data.get('game_id', 'неизвестно'))}\n"
        f"• Описание игрока в игре: {html.escape(game_description)}\n"
        ...
        f"• Дата регистрации в боте: {html.escape(player_data.get('registered_at', 'неизвестно'))}\n"
        f"• Дата последнего сообщения в боте: {html.escape(player_data.get('last_command_at', 'неизвестно'))}\n"
...
async def admin_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    nick_emoji = '<tg-emoji emoji-id="5255835635704408236">🗨️</tg-emoji>'
    time_emoji = '<tg-emoji emoji-id="5255971360965930740">🕓</tg-emoji>'
    ...
    text = (
        ...
        f"<b>Статистика</b> ⤵︎\n"
        f"- {nick_emoji} Зарегистрированный аккаунт: {html.escape(pdata.get('game_nick', 'неизвестно'))}\n"
        f"- {time_emoji} Дата регистрации: {html.escape(pdata.get('registered_at', 'неизвестно'))}\n"
        f"- {coins_emoji} Монеты: {html.escape(coins_str)}\n"
        f"- {tokens_emoji} Токены: {html.escape(tokens_str)}\n\n"
        f"<b>Информация</b> ⤵︎\n"
        f"- {flower_emoji} Время до конца администратора: {html.escape(time_left)}"
    )
...
async def referral_system(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"| Ваша ссылка: <code>{html.escape(ref_link)}</code>\n"
        ...
        f"<b>За каждого реферала:</b> {format_coins(REFERRAL_BONUS)} монет\n"
        f"<b>Количество рефералов:</b> {pdata.get('referral_count', 0)}\n"
        f"<b>Заработано монет:</b> {format_coins(pdata.get('referral_count', 0) * REFERRAL_BONUS)}"
    )
...
async def toggle_auto_friend_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"<b>Текущий статус:</b> {'✅ Да' if auto_friend else '❌ Нет'}\n"
        f"<b>За каждого реферала:</b> {format_coins(REFERRAL_BONUS)} монет\n"
        f"<b>Количество рефералов:</b> {players[str(user_id)].get('referral_count', 0)}\n"
        f"<b>Заработано монет:</b> {format_coins(players[str(user_id)].get('referral_count', 0) * REFERRAL_BONUS)}"
    )
    button_text = "✅ Да" if auto_friend else "❌ Нет"
    keyboard = [[InlineKeyboardButton(button_text, callback_data="toggle_auto_friend")]]
...
async def friend_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot_username = (await context.bot.get_me()).username
    friends_emoji = '<tg-emoji emoji-id="5255977030322760582">🧟‍♂️</tg-emoji>'
...
async def friend_profile_by_link(friend_nick: str, user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"👤 <b>Профиль друга</b>\n"
        f"{nick_emoji} Никнейм в игре: {html.escape(friend_data.get('game_nick', 'неизвестно'))}\n"
        f"{time_emoji} Время регистрации: {html.escape(friend_data.get('registered_at', 'неизвестно'))}\n"
        f"{coins_emoji} Монеты: {html.escape(coins_str)}\n"
        f"{tokens_emoji} Токены: {html.escape(tokens_str)}"
    )
...
async def item_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "withdraw":
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if trade_id:
                    ...
                    # Отправляем уведомление в чат выводов
                    ...
                    withdraw_text = (
                        ...
                        f"<tg-emoji emoji-id=\"5382194935057372936\">🕓</tg-emoji> <b>Время:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                        f"• <b>Игрок:</b> {html.escape(nick)} (@{html.escape(username)})\n"
                        f"• <b>Скин:</b> {html.escape(skin_name)}\n"
                        f"• <b>Сообщение в трейде:</b> <code>{html.escape(msg_code)}</code>\n"
                        f"• <b>ID трейда:</b> <code>{html.escape(trade_id)}</code>"
                    )
                    ...
                    # Сохраняем в белый список
                    add_to_whitelist(trade_id, msg_code, game_id, skins_offered, notification_msg_id=sent.message_id)
        ...
        # Выбран скин для обмена со стороны инициатора
        ...
        # Отправляем уведомление в чат виртуальных трейдов
        players = load_json(PLAYERS_FILE, {})
        ...
        text = (
            ...
            f"<tg-emoji emoji-id=\"5382194935057372936\">🕓</tg-emoji> <b>Время:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            f"• <tg-emoji emoji-id=\"5282843764451195532\">📁</tg-emoji> <b>Имена</b>\n"
            f"- Отправитель: {html.escape(initiator_nick)} - @{html.escape(initiator_username)}\n"
            f"- Получатель: {html.escape(target_nick)} - @{html.escape(target_username)}\n"
            f"• <tg-emoji emoji-id=\"5447152020442070774\">🔫</tg-emoji> <b>Скины</b>\n"
            f"*{html.escape(init_skin_name)}\n"
            f"=======================\n"
            f"*{html.escape(target_skin_name)}"
        )
...
async def exchange_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "info":
        ...
        # Ответ на сообщение в чате виртуальных трейдов
        if 'notification_msg_id' in exch:
            try:
                ...
                print(f"Не удалось ответить на сообщение о трейде: {e}")
...
def format_item_info(item):
    ...
    mod_name = get_modifier_name(item['modifier'])
    stickers = item.get('stickers', [])
    text = f"<b>{html.escape(skin_name)}</b> - {html.escape(mod_name)}\n"
    if stickers:
        text += "Наклейки:\n"
        for s in stickers:
            st_name = get_sticker_name(s['code'])
            text += f"  Слот {s['slot']}: {html.escape(st_name)}\n"
    else:
        text += "Наклеек нет\n"
    return text
...
async def show_user_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"👤 <b>Ваш профиль</b> ⤵︎\n"
        f"{line}\n"
        f"- {nick_emoji} Никнейм в игре: {html.escape(pdata.get('game_nick', 'неизвестно'))}\n"
        f"- {role_emoji} Роль: {role_display}\n"
        f"- {time_emoji} Время регистрации: {html.escape(reg_time)}\n"
        f"- {coins_emoji} Монеты: {html.escape(coins_str)}\n"
        f"- {tokens_emoji} Токены: {html.escape(tokens_str)}\n"
...
async def bd_upload_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "<tg-emoji emoji-id=\"5384223156053496423\">📁</tg-emoji> Вы хотите загрузить базу данных игроков.\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text in ["👤 Меню игрока", "⚙️ Админ-меню", "👤 Друзья", "⚡ Реферальная система", "🍪 Инвентарь", "🔄 Обмены", "📤 Исходящие", "📥 Входящие", "🧟‍♂️ Добавить друга", "👤 Список друзей", "🕓 Активные запросы", "◀️ Назад в профиль", "👤 Мой профиль (админ)", "🔍 Найти игрока", "◀️ Назад"]:
        if not is_registered(user_id):
            await update.message.reply_text("❌ Сначала зарегистрируйтесь через /start")
            return
...
def main():
    ...
    print("🚀 Бот с регистрацией, ролями, монетами, токенами, друзьями, рефералами, инвентарём, обменами и уведомлениями запущен.")
