...
# ============= КОНСТАНТЫ ДЛЯ ПРЕМИУМ ЭМОДЗИ =============
EMOJI = {
    'profile': '<tg-emoji emoji-id="5255835635704408236">👤</tg-emoji>',
    'crown': '<tg-emoji emoji-id="5217822164362739968">👑</tg-emoji>',
    'time': '<tg-emoji emoji-id="5255971360965930740">🕓</tg-emoji>',
    'coins': '<tg-emoji emoji-id="5409048419211682843">💰</tg-emoji>',
    'tokens': '<tg-emoji emoji-id="5427168083074628963">💎</tg-emoji>',
    'friends': '<tg-emoji emoji-id="5255977030322760582">🧟‍♂️</tg-emoji>',
    'referral': '<tg-emoji emoji-id="5456140674028019486">⚡</tg-emoji>',
    'flower': '<tg-emoji emoji-id="5456140674028019486">🌹</tg-emoji>',
    'folder': '<tg-emoji emoji-id="5384223156053496423">📁</tg-emoji>',
}
...
async def money_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"{EMOJI['coins']} Ваш баланс: {format_coins(coins)} монет", parse_mode='HTML')
...
async def tokens_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"{EMOJI['tokens']} Ваш баланс токенов: {tokens}", parse_mode='HTML')
    update_player_stats(user_id)
...
async def admin_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"{EMOJI['flower']} <b>Административный профиль</b>\n\n"
        f"<b>Статистика</b> ⤵︎\n"
        f"- {EMOJI['profile']} Зарегистрированный аккаунт: {pdata.get('game_nick', 'неизвестно')}\n"
        f"- {EMOJI['time']} Дата регистрации: {pdata.get('registered_at', 'неизвестно')}\n"
        f"- {EMOJI['coins']} Монеты: {coins_str}\n"
        f"- {EMOJI['tokens']} Токены: {tokens_str}\n\n"
        f"<b>Информация</b> ⤵︎\n"
        f"- {EMOJI['flower']} Время до конца администратора: {time_left}"
    )
...
async def referral_system(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"{EMOJI['referral']} <b>Реферальная система</b> ⤵︎\n"
    ...
    try:
        await context.bot.send_message(
            ...
            text=(
                ...
                f"{EMOJI['friends']} Ник отправителя: {sender_nick}\n\n"
...
async def friend_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot_username = (await context.bot.get_me()).username
    text = f"{EMOJI['friends']} <b>Список друзей</b> ⤵︎\n"
...
async def friend_profile_by_link(friend_nick: str, user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"{EMOJI['profile']} <b>Профиль друга</b>\n"
        f"{EMOJI['profile']} Никнейм в игре: {friend_data.get('game_nick', 'неизвестно')}\n"
        f"{EMOJI['time']} Время регистрации: {friend_data.get('registered_at', 'неизвестно')}\n"
        f"{EMOJI['coins']} Монеты: {coins_str}\n"
        f"{EMOJI['tokens']} Токены: {tokens_str}"
    )
    return text
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args and len(context.args) > 0:
        ...
        if param.startswith("friend_profile_"):
            ...
            result = await friend_profile_by_link(friend_nick, user_id, context)
...
async def show_user_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"{EMOJI['profile']} <b>Ваш профиль</b> ⤵︎\n"
        f"{line}\n"
        f"- {EMOJI['profile']} Никнейм в игре: {pdata.get('game_nick', 'неизвестно')}\n"
        f"- {EMOJI['crown']} Роль: {role_display}\n"
        f"- {EMOJI['time']} Время регистрации: {reg_time}\n"
        f"- {EMOJI['coins']} Монеты: {coins_str}\n"
        f"- {EMOJI['tokens']} Токены: {tokens_str}\n"
        f"- {EMOJI['friends']} Друзей: {friends_count}\n"
        f"- {EMOJI['referral']} Рефералов: {referral_count}\n"
        f"{line}"
    )
...
async def bd_upload_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"{EMOJI['folder']} Вы хотите загрузить базу данных игроков.\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "user":
        text = (
            f"{EMOJI['profile']} <b>Доступные команды (игрок):</b>\n\n"
        ...
        text = (
            f"{EMOJI['profile']} <b>Доступные команды (администратор):</b>\n\n"
            "<b>Мониторинг:</b>\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала проверяем загрузку базы данных
    ...
    if text in ["👤 Меню игрока", "⚙️ Админ-меню", "👤 Друзья", "⚡ Реферальная система", "🧟‍♂️ Добавить друга", "👤 Список друзей", "🕓 Активные запросы", "◀️ Назад в профиль", "👤 Мой профиль (админ)", "🔍 Найти игрока", "◀️ Назад"]:
        if not is_registered(user_id):
            await update.message.reply_text("❌ Сначала зарегистрируйтесь через /start")
            return
...
def main():
    ...
    print("🚀 Бот с регистрацией, ролями, монетами, токенами, друзьями, рефералами и базой данных запущен.")
