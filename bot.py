...
from premium_emoji import install_premium_emoji_patches
...
from scripts.sender_profile import (
    load_sender_profile_config,
    save_sender_profile_config,
    format_sender_profile_config,
    build_trade_offer_params,
)
...
async def withdraw_skin(user_id: int, item_id: str, context: ContextTypes.DEFAULT_TYPE):
    """Функция для вывода скина с визуализацией"""
    owner_id, item = get_item_owner(item_id)
    ...
    # Вспомогательная функция для обработки ошибки
    ...
    # Генерируем уникальный код для возможной ошибки заранее
    error_code = '#' + ''.join(random.choices(string.ascii_uppercase + string.digits, k=5))

    # Шаг 1: Сканирование ID
    ...
    # Шаг 2: Обход блокировки трейдов
    ...
    base_url = "https://api.efezgames.com/v1/trades/createOffer"

    # Шаг 3: Отправка трейда
    await status_msg.edit_text("💤 Отправка трейда...")

    try:
        random_token = generate_random_token()
        full_url = (f"{base_url}?token={random_token}"
                    f"&playerID={game_id}"
                    f"&receiverID="
                    f"&senderNick=EfezBot"
                    f"&senderFrame="
                    f"&senderAvatar="
                    f"&receiverNick="
                    f"&receiverFrame="
                    f"&receiverAvatar="
                    f"&skinsOffered={urllib.parse.quote(skin)}"
                    f"&skinsRequested="
                    f"&message={urllib.parse.quote(msg_code)}"
                    f"&pricesHash=fbd9aec4384456124c0765581a4ba099"
                    f"&receiverOneSignal="
                    f"&senderOneSignal="
                    f"&senderVersion=2.40.0"
                    f"&receiverVersion=2.40.0")

        logger.info(f"Отправка запроса к API: {full_url}")
        resp = requests.get(full_url, timeout=10)
        logger.info(f"Ответ API: статус {resp.status_code}, тело: {resp.text}")

        if resp.status_code != 200:
            raise Exception(f"API вернул статус {resp.status_code}: {resp.text[:200]}")
...
async def nuke_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        # Успех: редактируем сообщение и добавляем кнопку
        ...
        # Обрезаем, если слишком длинно (Telegram ограничение 4096)
...
# ============= ОБРАБОТЧИК КНОПКИ "👤 Посмотреть" ПОСЛЕ NUKE =============
...
async def senderprofile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sub in ("show", "status"):
        await update.message.reply_text(format_sender_profile_config(profile))
        return
    elif sub == "nick":
        ...
        try:
            profile["update_interval_seconds"] = max(10, int(rest))
        except ValueError:
            await update.message.reply_text("❌ interval должен быть числом")
            return
    elif sub == "schedule":
        profile["schedule_msk"]["enabled"] = rest.lower() in ("true", "1", "yes", "on", "да")
    elif sub == "start":
        if not re.match(r"^\d{2}:\d{2}$", rest):
            await update.message.reply_text("❌ Формат start: HH:MM")
            return
        profile["schedule_msk"]["start"] = rest
    elif sub == "end":
        if not re.match(r"^\d{2}:\d{2}$", rest):
            await update.message.reply_text("❌ Формат end: HH:MM")
            return
        profile["schedule_msk"]["end"] = rest
    elif sub == "addnick":
        if not rest:
            await update.message.reply_text("❌ Укажите ник")
            return
        profile["nick_cycle"].append(rest)
    elif sub == "delnick":
        try:
            idx = int(rest) - 1
            removed = profile["nick_cycle"].pop(idx)
            save_sender_profile_config(SENDER_PROFILE_FILE, profile)
            await update.message.reply_text(f"✅ Удалён ник: {removed}")
            return
        except Exception:
            await update.message.reply_text("❌ Неверный номер ника")
            return
    elif sub == "addmsg":
        if not rest:
        ...
        try:
            idx = int(rest) - 1
            removed = profile["message_cycle"].pop(idx)
            save_sender_profile_config(SENDER_PROFILE_FILE, profile)
            await update.message.reply_text(f"✅ Удалено сообщение: {removed}")
            return
        except Exception:
            await update.message.reply_text("❌ Неверный номер сообщения")
            return
    else:
        await update.message.reply_text("❌ Неизвестная подкоманда /senderprofile")
        return
...
def main():
    ...
    install_premium_emoji_patches(app)
