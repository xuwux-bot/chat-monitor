...
def get_player_by_nick(nick: str, players: dict) -> Optional[str]:
    ...
    for tid, pdata in players.items():
        ...
        logger.info(f"Проверяю игрока {tid}: game_nick={game_nick}")
        if game_nick and game_nick.lower() == nick.lower():
            logger.info(f"Найдено совпадение: {tid}")
            return tid
    logger.info(f"Игрок с ником {nick} не найден")
    return None
...
# ============= ФУНКЦИЯ ФОРМАТИРОВАНИЯ ЦЕННОСТИ СКИНА =============
def format_skin_value(modifier: int) -> str:
    """
    Преобразует числовой модификатор в читаемое описание ценности скина.
    Примеры:
        40 -> "Прямо с завода"
        42 -> "Прямо с завода Сувенирный"
        44 -> "Статрек Прямо с завода"
        46 -> "Статрек Прямо с завода Сувенирный"
    """
    mod_name = MODIFIER_NAMES.get(modifier, "")
    if not mod_name:
        return f"модификатор {modifier}"
    # mod_name может быть, например: "Стартрек (Сувенирный)-[прямо с завода]"
    parts = mod_name.split('-')
    if len(parts) != 2:
        return mod_name
    prefix = parts[0].strip()      # например "Стартрек (Сувенирный)"
    condition = parts[1].strip('[]')  # например "прямо с завода"

    # Определяем наличие StatTrak и Souvenir
    has_statrak = "Стартрек" in prefix
    has_souvenir = "Сувенирный" in prefix

    result = condition
    if has_souvenir:
        result += " Сувенирный"
    if has_statrak:
        result = "Статрек " + result
    return result
...
async def admin_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game_nick = pdata.get('game_nick', 'неизвестно')
    reg_date = pdata.get('registered_at', 'неизвестно')
...
async def skin_add_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2:
        ...
        return
    # Исправлено: ник — это args[1] (после подкоманды add)
    target_nick = args[1]
    items_str = ' '.join(args[2:])
    raw_items = [s.strip().strip('"') for s in items_str.split(',') if s.strip()]
    players = load_json(PLAYERS_FILE, {})
    logger.info(f"Загружены игроки: {list(players.keys())}")
    target_id = get_player_by_nick(target_nick, players)
...
async def show_item_menu(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    skin_name = get_skin_name(item['skin_code'])
    skin_value = format_skin_value(item['modifier'])
...
async def item_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "withdraw":
        # Получаем game_id владельца
        players = load_json(PLAYERS_FILE, {})
        player_data = players.get(owner_id)
        if not player_data:
            await query.edit_message_text("❌ Данные игрока не найдены.")
            return
        game_id = player_data.get('game_id')
        if not game_id:
        ...
        params = {
            ...
            "playerID": game_id,                     # !!! ВАЖНО: используем game_id, не telegram_id
            "receiverID": "",
            "senderNick": "EfezBot",
            "senderFrame": "",
            "senderAvatar": "",
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if trade_id:
                    # Отправка уведомления в чат выводов
