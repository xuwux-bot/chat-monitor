...
def build_admin_root_menu_markup():
    keyboard = [
...
async def show_admin_root_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    reply_markup = build_admin_root_menu_markup()
...
async def back_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin_or_owner(query.from_user.id):
        await context.bot.send_message(chat_id=query.from_user.id, text="Выберите меню:", reply_markup=build_admin_root_menu_markup())
    else:
...
async def handle_reply_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "◀️ Назад в профиль":
        ...
        if is_admin_or_owner(user_id):
            await show_admin_root_menu(update, context)
        else:
            await show_user_profile(update, context)
        return
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if update and getattr(update, 'effective_message', None):
            await update.effective_message.reply_text("❌ Произошла ошибка обработки команды. Попробуйте ещё раз.")
    except Exception:
        pass
