...
LOG_DIR = "logs"
DOWNLOAD_LIMIT = 100  # максимальное количество сообщений при ручной загрузке
...
# Импортируем функции из модуля report
from scripts.report import (
    handle_report_command,
    handle_report_status_command,
    stop_report_task,
    stop_all_reports,
    get_report_stats
)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "/stop – остановить задачу (например /stop Мониторинг RU или /stop Report 123456)\n"
        "/status – статус мониторинга\n"
        "/download КАНАЛ [количество] – принудительно загрузить последние сообщения из канала (только при активном мониторинге)\n"
        "   Пример: /download DEV 40\n"
        "/report <айди игрока> <причина> <количество> – начать отправку жалоб на игрока\n"
        "   Причины: 0 – неуместный ник, 1 – неуместное сообщение, 2 – спам\n"
        "   Количество: 0 – бесконечно (остановить через /stop Report <айди>)\n"
        "/report status – показать статистику активных жалоб (сообщение обновляется)\n"
        "/report stop – остановить все активные жалобы\n"
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Укажи имя задачи: /stop <имя> (например /stop Мониторинг RU или /stop Report 123456)")
        return

    task_name = ' '.join(context.args)

    # Проверяем, не жалоба ли это
    if task_name.startswith("Report "):
        target_id = task_name[7:].strip()
        if stop_report_task(target_id):
            await update.message.reply_text(f"✅ Жалобы на игрока {target_id} остановлены.")
        else:
            await update.message.reply_text(f"❌ Активная задача Report {target_id} не найдена.")
        return

    # Проверяем, не спам ли это (если есть)
    if task_name.startswith("Спам "):
        channel = task_name[5:].strip()
        # если есть функция остановки спама, вызвать её
        # пока заглушка
        await update.message.reply_text(f"❌ Остановка спама не реализована.")
        return
...
async def report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("Использование: /report <айди игрока> <причина> <количество> или /report status или /report stop")
        return

    if args[0].lower() == "status":
        await handle_report_status_command(update, context)
    elif args[0].lower() == "stop":
        # Останавливаем все активные жалобы
        stopped = stop_all_reports()
        if stopped:
            await update.message.reply_text(f"✅ Остановлено {stopped} активных жалоб.")
        else:
            await update.message.reply_text("Нет активных жалоб для остановки.")
    else:
        # Обычный запуск жалобы
        await handle_report_command(update, context, active_tasks)
...
def main():
    ...
    print("🚀 Монитор-бот с логами, каналом DEV и жалобами запущен. Нажми Ctrl+C для остановки.")
    app.run_polling()
