...
# Для управления flood wait по каждому чату
flood_until: Dict[Tuple[int, int], float] = {}  # (chat_id, thread_id) -> время, до которого нельзя слать
...
def parse_telegram_link(link: str) -> Optional[Tuple[int, int]]:
    """Извлекает chat_id и thread_id из ссылки вида https://t.me/c/123456789/10."""
...
def extract_nick_from_text(text: str) -> Optional[str]:
    """Извлекает ник из сообщения вида '[12:34] [Nick]: text'."""
...
def get_player_nick(player_id: str) -> Optional[str]:
    """Получает ник игрока по его ID через API /social/findUser."""
...
async def safe_send_message(bot, chat_id: int, text: str, thread_id: int = None) -> bool:
    """Отправка сообщения в Telegram с учётом flood wait."""
...
# ============= ПОЛНАЯ ФУНКЦИЯ ОТПРАВКИ ОТВЕТА (с подтверждением) =============
async def send_reply(update: Update, context: ContextTypes.DEFAULT_TYPE, nick: str, channel: str, user_text: str, lang: str = None):
    ...
    if success:
        # ✅ Подтверждение в чат (то самое, которое просили вернуть)
        await update.message.reply_text(f"✅ Ответ отправлен игроку {nick} в канал {channel}")
        
        # 🎮 Игровое сообщение для видимости в ленте (как от других игроков)
        sender_nick = get_player_nick(sender_id) or sender_id
        thread_id = update.message.reply_to_message.message_thread_id if update.message.reply_to_message else None
        game_formatted = f"[{datetime.now().strftime('%H:%M:%S')}] [{sender_nick}]: {reply_text}"
        await safe_send_message(context.bot, chat_id, game_formatted, thread_id)
    else:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "/help – это сообщение\n\n"
        "📝 **Как отвечать игрокам:**\n"
        "• Ответь (reply) на любое сообщение, пересланное ботом.\n"
        "• Для обычных каналов ответ отправится автоматически.\n"
        "• Для PREMIUM-канала бот спросит, на каком языке ответить (RU или US).\n\n"
        "Доступные игровые каналы: RU, UA, US, PL, DE, PREMIUM"
    )
...
async def setlink_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2:
        ...
        return
    game = args[0].upper()
    allowed = ["RU", "UA", "US", "PL", "DE", "PREMIUM"]
...
def main():
    ...
    print("🚀 Монитор-бот с ответами и сменой ID запущен. Нажми Ctrl+C для остановки.")
    app.run_polling()
