...
async def referral_system(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Показывает реферальную информацию и настройки."""
    user_id = update.effective_user.id
    players = load_json(PLAYERS_FILE, {})
    pdata = players.get(str(user_id))
    if not pdata:
    ...
    auto_status = "✅ Да" if auto_friend else "❌ Нет"
    ...
    keyboard = [
        [InlineKeyboardButton(f"{'✅' if auto_friend else '❌'} Да / Нет", callback_data="toggle_auto_friend")]
    ]
...
async def toggle_auto_friend_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Переключает настройку автоматического добавления в друзья."""
    ...
    # Обновляем сообщение
    await referral_system(update, context)  # но это callback, нужно перезапустить или отредактировать
    # Проще отредактировать:
    ...
    keyboard = [[InlineKeyboardButton(f"{'✅' if auto_friend else '❌'} Да / Нет", callback_data="toggle_auto_friend")]]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Проверяем, не передан ли реферальный код
    if context.args and len(context.args) > 0:
        ref_code = context.args[0]
        # Сохраняем в user_data, чтобы использовать при регистрации
...
async def confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result and result.get("success"):
        ...
        # Проверяем, был ли передан реферальный код при старте
        ...
        if referrer_code:
            ...
            for tid, pdata in players.items():
                if pdata.get('referral_code') == referrer_code:
                    referrer_id = tid
                    # Проверяем, что пользователь не пытается активировать свою ссылку
        ...
        # Если есть реферер, начисляем бонус и, возможно, отправляем запрос в друзья
        if referrer_id:
            # Начисляем бонус
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=f"🎉 По вашей реферальной ссылке зарегистрировался новый пользователь {game_nick}!\n"
                         f"💰 Вам начислено {format_coins(REFERRAL_BONUS)} монет."
                )
            except:
                pass

            # Если у реферера включено авто-добавление, отправляем запрос в друзья
            if players[referrer_id].get('auto_add_friend', True):
                # Отправляем запрос в друзья от реферера новому пользователю
                referrer_nick = players[referrer_id].get('game_nick')
                if referrer_nick:
                    # Добавляем запрос в friend_requests нового пользователя
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обработка reply-клавиатуры (доступна только зарегистрированным)
    ...
    # Обработка разблокировки трейдов (доступна только зарегистрированным)
    if await handle_unblock_reply(update, context):
        return

    # Проверяем, зарегистрирован ли пользователь (для остальных действий)
    if not is_registered(user_id):
        # Если не зарегистрирован и сообщение не команда, игнорируем
        return
