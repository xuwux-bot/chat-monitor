...
# Чат для уведомлений о трейдах (именно из твоей ссылки)
...
active_tasks: Dict[str, asyncio.Task] = {}  # обязательно!

flood_until: Dict[Tuple[int, int], float] = {}

# Импортируем модуль блокировки трейдов
    ...
    if subcmd == "trade":
        if len(args) == 1:
            ...
            if blocker_is_running():
                ...
                return
            # Передаём бота и параметры чата для уведомлений
...
def main():
    ...
    print("🚀 Монитор-бот с блокировкой трейдов запущен. Нажми Ctrl+C для остановки.")
    app.run_polling()
