...
import requests
import aiohttp
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup, ReplyKeyboardMarkup, KeyboardButton, MessageEntity, Message, CallbackQuery, Bot
...
_PREMIUM_EMOJI_KEYS = sorted(PREMIUM_EMOJI_MAP.keys(), key=len, reverse=True)

def _utf16_len(value: str) -> int:
    return len(value.encode("utf-16-le")) // 2

def _build_premium_entities(text: str, entities=None):
    ...
    for emoji in _PREMIUM_EMOJI_KEYS:
        ...
        while True:
            ...
            if idx == -1:
                break
            span = range(idx, idx + len(emoji))
            if not any(pos in occupied for pos in span):
                found.append((idx, emoji))
                for pos in span:
                    occupied.add(pos)
            start = idx + len(emoji)

    for idx, emoji in sorted(found, key=lambda item: item[0]):
    ...
    return text, new_entities or None

def _extract_button_icon(text: str):
    if not isinstance(text, str):
        return text, None
    stripped = text.lstrip()
    lead_spaces = text[: len(text) - len(stripped)]
    for emoji in _PREMIUM_EMOJI_KEYS:
        if stripped.startswith(emoji):
            rest = stripped[len(emoji):].lstrip()
            cleaned = lead_spaces + rest
            if cleaned.strip():
                return cleaned, PREMIUM_EMOJI_MAP[emoji]
            return text, PREMIUM_EMOJI_MAP[emoji]
    return text, None

def _clone_inline_button(button: InlineKeyboardButton):
    text, icon_id = _extract_button_icon(button.text)
...
def _clone_reply_button(button: KeyboardButton):
    text, icon_id = _extract_button_icon(button.text)
    api_kwargs = dict(getattr(button, "api_kwargs", {}) or {})
    if icon_id:
        api_kwargs["icon_custom_emoji_id"] = icon_id
    kwargs = {
        ...
        "request_user": getattr(button, "request_user", None),
        "api_kwargs": api_kwargs or None,
    }
    kwargs = {k: v for k, v in kwargs.items() if v is not None}
    return KeyboardButton(text=text, **kwargs)

def _premiumize_reply_markup(reply_markup):
    ...
    try:
        if isinstance(reply_markup, InlineKeyboardMarkup):
            ...
            for row in reply_markup.inline_keyboard:
                new_keyboard.append([
                    _clone_inline_button(button) if isinstance(button, InlineKeyboardButton) else button
                    for button in row
                ])
        ...
        if isinstance(reply_markup, ReplyKeyboardMarkup):
            ...
            for row in reply_markup.keyboard:
                new_keyboard.append([
                    _clone_reply_button(button) if isinstance(button, KeyboardButton) else button
                    for button in row
                ])
            return ReplyKeyboardMarkup(
                ...
                input_field_placeholder=getattr(reply_markup, "input_field_placeholder", None),
                is_persistent=getattr(reply_markup, "is_persistent", None),
            )
    except Exception as e:
        logger.error(f"Ошибка premium emoji в клавиатуре: {e}")
    return reply_markup
...
async def _premium_reply_text(self, text, *args, **kwargs):
    text, entities = _build_premium_entities(text, kwargs.get("entities"))
    kwargs["entities"] = entities
    kwargs["reply_markup"] = _premiumize_reply_markup(kwargs.get("reply_markup"))
...
async def _premium_bot_edit_message_text(self, text, *args, **kwargs):
    text, entities = _build_premium_entities(text, kwargs.get("entities"))
    kwargs["entities"] = entities
    kwargs["reply_markup"] = _premiumize_reply_markup(kwargs.get("reply_markup"))
    return await _original_bot_edit_message_text(self, text, *args, **kwargs)

async def _premium_callback_edit_message_text(self, text, *args, **kwargs):
    text, entities = _build_premium_entities(text, kwargs.get("entities"))
    kwargs["entities"] = entities
    kwargs["reply_markup"] = _premiumize_reply_markup(kwargs.get("reply_markup"))
    return await _original_callback_edit_message_text(self, text, *args, **kwargs)

Message.reply_text = _premium_reply_text
Message.edit_text = _premium_edit_text
Bot.send_message = _premium_send_message
Bot.edit_message_text = _premium_bot_edit_message_text
CallbackQuery.edit_message_text = _premium_callback_edit_message_text
# ==============================================
...
awaiting_withdraw_skin: Dict[int, str] = {}
...
def load_item_names():
    ...
    logger.info(f"Загружено предметов: {len(ITEM_NAMES)}")
...
async def inventory_navigation_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data[0] == "back_to_friend_exchange":
        await show_friends_for_exchange_callback(update, context)
...
def send_skin_to_game(player_id: str, skin: str) -> Tuple[bool, Optional[str], str]:
    try:
        ...
        logger.info(f"Отправка скина: playerID={player_id}, skin={skin[:50]}...")
        response = requests.get(url, params=params, timeout=30)
        resp_text = response.text
        logger.info(f"Ответ API: {resp_text}")
        ...
        if data.get('success') or '"success":true' in resp_text:
            trade_id = data.get('offerID') or data.get('_id') or data.get('id')
            if trade_id:
                return True, trade_id, "Успешно"
            else:
                return True, None, "Успешно (ID не получен)"

        lower = resp_text.lower()
        if "another trade active" in lower:
            return False, None, "❌ У вас уже есть активный трейд! Примите или отклоните предыдущий трейд в игре."
        if "access denied" in lower:
            logger.warning("API вернул access denied, но трейд возможно создан")
            return True, None, "Успешно (игнорируя ошибку авторизации)"

        return False, None, f"❌ Ошибка API: {resp_text[:200]}"

    except requests.exceptions.Timeout:
        return False, None, "⏱️ Время ожидания истекло. Проверьте подключение."
...
async def withdraw_skin(user_id: int, item_id: str, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def handle_error(step: str, error: Exception, error_code: str):
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text=(
                f"❗Произошла ошибка вывода скина!\n"
                f"Администратор получил уведомление.\n"
                f"•Пожалуйста, если вы не получили скин обратитесь к администратору - @xuwyx\n"
                f"Код ошибки {error_code} (сообщите этот код админу)"
            )
        )
        logger.error(f"Ошибка вывода: {error_code} - {error}")
    ...
    try:
        ...
        await handle_error("получение game_id", e, error_code)
        return
    ...
    try:
        success, trade_id, result_msg = send_skin_to_game(game_id, skin)
        if not success:
        ...
        withdraw_text = (
            ...
            f"• 🔫 Скин: {skin_name}\n"
            f"• Сообщение в трейде: {msg_code}\n"
        )
        if trade_id:
            withdraw_text += f"• ID трейда: {trade_id}"
        try:
        ...
        if trade_id:
            add_to_whitelist(trade_id, msg_code, game_id, skin, notification_msg_id=notification_id)
            asyncio.create_task(auto_reject_trade(trade_id, game_id))
...
async def show_user_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"👤 Ваш профиль ⤵︎\n"
        f"{line}\n"
        f"- {nick_emoji} Никнейм в игре: {pdata.get('game_nick', 'неизвестно')}\n"
        f"- {role_emoji} Роль: {role_display}\n"
        f"- {time_emoji} Время регистрации: {reg_time}\n"
        f"- {coins_emoji} Монеты: {coins_str}\n"
        f"- {tokens_emoji} Токены: {tokens_str}\n"
...
async def handle_reply_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "◀️ Назад в профиль":
        ...
        if friend_nick:
            ...
            await show_user_profile(update, context)
        return
    elif text == "◀️ Назад к выбору друга":
        await show_friends_for_exchange(update, context)
        return
    elif text == "◀️ Назад":
...
async def confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result and result.get("success"):
        ...
        players = load_json(PLAYERS_FILE, {})
        # Проверка на уникальность game_id
        for tid, pdata in players.items():
            if pdata.get('game_id') == game_id:
                await update.message.reply_text("❌ Этот игровой ID уже зарегистрирован другим пользователем.")
                return
...
def extract_nick_from_text(text: str) -> Optional[str]:
    ...
    match = re.search(r'\|\s*Ник отправителя:\s*(.+)', text)
    if match:
...
async def nuke_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        text = "✅ Готово! Данные пользователя сброшены."
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("👤 Посмотреть", callback_data=f"nuke_view|{player_id}")]
        ])
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if role == "user":
        ...
        text = (
            ...
            "/edit (ответом на сообщение бота) - изменить текст сообщения\n\n"
            "Остановка задач:\n"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(item_action_callback, pattern='^item\|(withdraw|delete|exchange|exchange_from_friend|send_exchange)'))
