...
async def item_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "withdraw":
        ...
        async def handle_error(step: str, error: Exception, error_code: str):
            ...
            user_info = f"Пользователь: {query.from_user.id} (@{query.from_user.username})"
        ...
        await query.edit_message_text("❗Сканирование айди...")
        ...
        await query.edit_message_text("⚙️ Обход блокировки трейдов...")
        ...
        await query.edit_message_text("💤 Отправка трейда...")
        ...
        try:
            ...
            await query.edit_message_text("✅ Скин успешно отправлен! Заходите в игру)")
