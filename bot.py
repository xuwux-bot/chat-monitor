...
def build_profile_text_for_viewer(target_id: str, viewer_id: Optional[int] = None, self_profile: bool = False) -> str:
    ...
    if role == 'owner':
        ...
        role_display = 'Игрок'
    header = '👤 Ваш профиль' if self_profile else f"👤 Профиль игрока {pdata.get('game_nick', 'неизвестно')}"
    tg_nick = pdata.get('tg_username')
    if tg_nick:
        display_nick = f"@{tg_nick}"
    else:
    ...
    lines.extend([
        f"- 👤 Никнейм - «{display_nick}»",
        f"- 👑 Привелегия - «{role_display}»",
        '• Информация',
        f"- 💤 Последний онлайн - «{get_online_text_for_viewer(str(target_id), viewer_id=viewer_id, self_profile=self_profile)}»",
        f"- 🕓 Зарегестрирован - «{format_short_date(pdata.get('registered_at'))}»",
        '• Статистика',
        f"- ⚡ Рефералов - «{pdata.get('referral_count', 0)}»",
        f"- 🧟‍♂️ Друзей - «{len(pdata.get('friends', []))}»",
        f"- 🍪 Скинов - «{get_inventory_count_for_player(str(target_id))}»",
        f"- 💰 Баланс - «{format_coins(pdata.get('coins', 0))}»",
        f"- 💎 Токены - «{pdata.get('tokens', 0)}»",
        '• Остальная информация',
        f"- Ник в игре - «{pdata.get('game_nick', 'неизвестно')}»",
        f"- Айди в игре - «{pdata.get('game_id', 'неизвестно')}»",
        '——————————————————————',
    ])
    return '\\n'.join(lines)
...
async def show_user_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        await context.bot.send_message(chat_id=update.callback_query.from_user.id, text=text, reply_markup=reply_markup, parse_mode='HTML')
    else:
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(exchange_open_callback, pattern='^exchange_open\|'))
    ...
    app.add_handler(CallbackQueryHandler(confirm_view_profile_callback, pattern='^viewprofile_confirm\|'))
